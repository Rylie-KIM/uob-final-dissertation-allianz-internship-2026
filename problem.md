...
- **Primary Layer-1 detector (added 2026-08-02): the error-inheritance test.** Mobile band rows garage-verified as *repairable* are rows where a high v1 score is **known** to have been wrong; if v2 trained on v1's forced labels it should **over-score exactly those rows**. Estimated as a local-linear jump in E[s_v2 | s_v1] at 0.75 on the mobile verified Y=0 population, with the Y=1 population as a second difference and placebo cutoffs as falsification. Needs no common feature space (scores joined by claim id only). Spec: `paper.mid.draft.md` §3.3.1; implementation: `notebook/real/01_error_inheritance.ipynb`. ψ (p29 residual density) is demoted to a secondary/corroborating signal.
  > **⚠️ CONFIRMED structurally inestimable on real data (2026-08-27 mobility finding, notebook fix 2026-09-11).** The `MOBILE` covariate this whole test keys on lived only in v1's production log, which is destroyed, and is confirmed absent from the raw dataset and training matrix too — there is nothing left to join, at any row count. This is not the "reconstruct from surviving artefacts" fallback §"the v1 → v2 hand-off loses its observational side" (below) anticipated; it is a closed dead end. `notebook/real/detection/02_error_inheritance.ipynb` §§1-5 now detect this and print the diagnosis as the v1→v2 result ("not estimable — permanently unrecoverable", never "underpowered"). **The v2→v3 near-boundary analogue (same notebook §6) is promoted to the real-data primary test** — it needs no segmenting covariate, since v2 carries an actual production log with recorded decisions.
...
- **Reporting lag is not constant** — plausibly varying by channel (FNOL vs ENOL, §1.2) and severity — so sorting by `ReportedDate` **reshuffles** the loss-date ordering, and the OOT block can contain *older losses reported late*. **🚫 Scope decision (2026-09-13): channel case-mix is explicitly out of scope for this thesis** (see §2.5 difficulty 6) — stated as a limitation, not quantified.
...
>
> **Enforced in the tooling (2026-08-01; notebook restructured 2026-08-18 into three runs — train / OOT / path-dependent-backend — so this table is now computed once, not per run).** That last requirement is no longer a discipline to remember: `src/scoring/attribute.py` captures the estimator's hyperparameters into `detection/shap/<v>/<v>_attributions_<split>_meta.json` — the only process that can open the pickle — and `notebook/real/00_shap_attribution.ipynb` §3b renders them beside the concentration table, flagging exactly which knobs differ across the versions being compared. It does **not** resolve the confound; options (i)–(iii) still stand. It only makes it impossible to report the concentration numbers without the configuration next to them.
...
6. **ENOL/FNOL channel-mix shift as a confound (confirmed 2026-06-22; 🚫 declared OUT OF SCOPE 2026-09-13):** Claims are filed either by phone (FNOL — First Notification of Loss) or online (ENOL — Electronic Notification of Loss, recently introduced). The two channels produce structurally different feature distributions: FNOL claims are handler-mediated and may capture more severe incidents (claimants in worse situations tend to phone rather than self-serve online), while ENOL claims follow a fixed form path with different missing-value and error patterns. If the proportion of ENOL claims grows over time as online filing becomes more common, this induces a feature distribution shift that is independent of any SFP loop. Score drift attributed to SFP amplification must therefore be checked against the ENOL/FNOL mix change as an alternative explanation. See `README.md` § "Claim Intake Channels — FNOL vs ENOL" for the full operational context. **Scope decision (2026-09-13): this confound is acknowledged but deliberately not measured or studied** — no canonical channel column exists anywhere in `src/` (would need a real column name off the company laptop) and judged not a major driver relative to the time available. State it as a limitation wherever channel-mix could plausibly matter; do not revisit without a specific reason.
