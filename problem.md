...
- **The *form* (absolute cutoff tuned to precision ≥ 0.985) is shared; the threshold *value* τ_v is tuned per-version** — `0.872` is v2's documented real instance; v1/v3 differ because each model's score distribution requires a different cutoff to hold ≥ 0.985. Synthetic generator tunes τ_v per version, landing near the anchor (v1 ≈ 0.867, v2a ≈ 0.921).
...
|---|---|---|---|---|---|---|
| v1 | `pre_ml_label` (human era) | ~12.9% | 0.986 | **0.914** | 0.072 | Deployed (superseded) |
| **v2a** *(real)* | `model_v1_observed_outcome` | **~19.2%** ↑ | 0.995 | **0.897** ↓ | 0.098 | **Currently deployed** |
| v2b *(counterfactual)* | mixed (pre-ML + v1 log) | ~19.1% | 0.995 | 0.897 | 0.098 | Synthetic only — not real |
| v3a *(SFP deepening)* | `model_v2a_observed_outcome` | **~20.0%** ↑ | 0.985 | **0.888** ↓ | 0.097 | Attempted; not deployed (real) |
| v3b *(counterfactual)* | `model_v2b_observed_outcome` | ~19.7% | 0.990 | 0.892 | 0.098 | Synthetic only — not real |
...
- **True (oracle) precision degrades**: 0.914 → 0.897 → 0.888 (v1→v2a→v3a), while contaminated precision looks flat/high — the *gap* (0.072 → 0.098) is the hidden false-positive inflation.
- **Scrap rate inflates** (~12.9% → ~20.0%) even though true repairability never changed, and the **true false-positive rate among scrapped cars rises** (8.6% → 11.2%): more genuinely-repairable cars are scrapped each generation.

Consequence for detection — what each candidate signal is worth here:
- **τ_v (threshold trajectory): unreliable.** Endogenous (a *response* to the contaminated data), non-monotonic (0.867 → 0.921 → 0.887). Cannot be read as an SFP indicator.
...
- **Scrap-rate inflation: a visible symptom, but confounded.** It *does* rise (~12.9% → ~20%) because holding contaminated precision does not constrain scrap *volume* — so it is not "absorbed". But rising scrap rate can equally come from case-mix / score-distribution shift unrelated to SFP, so it flags a symptom without proving harm.
- **Oracle precision degradation + contaminated-vs-oracle gap: the dispositive signal.** Directly measures the SFP harm (repairable cars wrongly scrapped). Available only in the synthetic DGP (`verify_sfp_oracle.py`), never operationally — which is exactly why the real detection framework (Build 02/03) needs the IPS / selective-labels machinery to *estimate* this gap rather than read it directly.
...
> oracle-precision degradation + scrap inflation rather than recall collapse, because the loop's
> contamination (~1.5–11% false positives among scrapped, anchored by the true garage labels of the
