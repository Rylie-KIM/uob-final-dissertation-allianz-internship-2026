...
**`src/scoring/retrain.py`** — version-agnostic **adapter over the version repo's own trainer**, run *inside* the target version's env exactly like `predict.py`. It invokes that version's `train.py` (the repo owns the Allianz protocol) on the version's **fixed** feature file plus a **supplied `--labels` file**. The `--labels` value is the *only* thing that differs between the baseline run (original contaminated target → `models/<source>/baseline/v<k>.pkl`) and the post-mitigation run (the corrector's de-contaminated target → `models/<source>/mitigated/v<k>.pkl`); features and procedure are held fixed, so the before→after score difference is attributable to the mitigation (the re-evaluation invariant, `problem.md` §2.5 #12). The sketch below is a **standalone illustration** of the swappable-label contract; on real/practice data the fit call delegates to the repo's trainer rather than re-implementing it.
...
- **Training → repo-owned protocol, thin retrain adapter.** Every version used the *same* Allianz methodology (2-month maturation exclusion → 6-month OOT → 80/20 fit/val → fit → tune τ to precision ≥ 0.985) — but that methodology is **implemented in each version's own repo `train.py`**, not re-implemented in `src/`. `retrain.py` is a **version-agnostic adapter** that runs the repo's training with a **swappable `--labels`** file: pass the *original* target → the baseline pkl (production reproduction); pass the *corrector's* target → the mitigated pkl. Everything else (features, procedure) is held fixed, so the only difference between the two runs is the label (the re-evaluation invariant, §2.5 #12). `src/training/spec.py` carries just the per-version **params** (label column, training window, hyperparameters, τ fallback) fed to that run. Adding a version = a spec entry + pointing at its repo, not new training code.
