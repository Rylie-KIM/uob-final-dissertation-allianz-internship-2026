...
>
> **env-v1 gets a separate pair (2026-08-24): `src/shap_kit_v1.py` + `notebook/real/00_SHAP_v1.ipynb`.** The library floor above holds for v2/v3 only — `shap_kit.py` (like `config.py` and `figstyle.py`) is 3.7+ *syntax* (f-strings, `from __future__ import annotations`), so on env-v1's Python 3.5.6 the import is a SyntaxError before a line runs. The `_v1` twin follows the `extract_features_v1.py` rule: 3.5 syntax, no config/figstyle import (v1 constants and CSV path builders live in the module), CSV output instead of parquet.
>
> **env-v1 CAN run `shap` after all — verified on the company laptop 2026-08-24.** `shap==0.35.0` is the last release with a **cp35 win_amd64 wheel**, it accepts `feature_perturbation="interventional"`, and its `XGBTreeModelLoader` parses the pre-1.0 raw model buffer xgboost 0.72 writes (shap's `xgboost < 0.81` guard fires only for `model_output != "raw"`, which we never ask for). Install it with **`--no-deps`** — it declares numpy/scipy/scikit-learn/pandas and pip would otherwise move this frozen env's pins — plus `tqdm==4.64.1`, the last tqdm whose `requires_python` admits 3.5, because `shap/__init__.py` imports KernelExplainer, which imports tqdm at module level: without it `import shap` fails outright. **`src/envs/v1/check_shap.py` is the gate** — it proves interventional runs, that additivity holds, and that the two backends are not the same numbers, before any notebook is changed. So `shap_kit_v1.compute()` now takes a `background=` and prefers the `shap` route (backend `"shap"`, matching v2/v3's sidecar meta so `require_comparable` accepts the comparison), with the booster's `pred_contribs` as the fallback when `shap` is absent — an interventional request can never be served by the booster, it raises instead.
>
> **Two v1-only traps this uncovered.** (1) v1's pickle needs a LOADER LADDER — it was serialised on sklearn < 0.23's vendored `sklearn.externals.joblib`, and plain `joblib.load` desyncs on that stream and dies in `pickle.py` with `KeyError: <n>`; `shap_kit_v1._load_any` now tries the same four loaders `features/extract_features_v1.py` proved. (2) **xgboost 0.72's sklearn wrapper ignores `output_margin=True`** (fixed upstream in 0.81 — the same bug shap guards against) and returns probabilities, so every additivity check compared log-odds against a probability and failed by ~7. `shap_kit_v1.model_margin()` asks the BOOSTER instead, carrying `best_ntree_limit` so early stopping cannot make the predicted margin cover a different tree set than the attribution summed.
>
> **`shap` is still optional, with one degraded feature when it is absent.** Values fall back to the booster's `pred_contribs`; **interaction** values fall back to `pred_interactions`, which arrived in **xgboost 0.81** — so on env-v1 (0.72) there is no native route and the interaction section needs `shap` installed there or it is skipped (the notebook says so and falls back to a correlation proxy for its dependence-plot colours). Installing `shap` into a version env stays preferable to relaxing any numeric pin.
>
...
**`src/scoring/attribute.py`** — the planned sibling of `predict.py`, emitting per-row SHAP attributions to `detection/shap/<v>/<v>_attributions_<split>.parquet` for the `estimator/` layer — runs in the **per-version env**, for the same reason `predict.py` does: SHAP needs the model *object*, and a pkl only unpickles where its repo is importable (loading `models/*/baseline/v1.pkl` from the analysis `.venv` raises `ModuleNotFoundError: fttl_v1`). That has a concrete env consequence:
