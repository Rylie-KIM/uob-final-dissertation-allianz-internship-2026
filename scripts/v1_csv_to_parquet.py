"""v1 CSV → parquet. Runs in the analysis `.venv`, NOT in env-v1.
    ...
    uv run python scripts/v1_csv_to_parquet.py                 # convert everything v1 exported
    uv run python scripts/v1_csv_to_parquet.py --dry-run       # list what would be converted
    uv run python scripts/v1_csv_to_parquet.py --splits train  # just one split
